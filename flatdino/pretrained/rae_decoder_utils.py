...
def _gelu_exact(x: Array) -> Array:
    """Exact GELU activation (matches PyTorch's default F.gelu)."""
    return jax.nn.gelu(x, approximate=False)


def _gelu_approximate(x: Array) -> Array:
    """Approximate GELU activation (faster but less precise)."""
    return jax.nn.gelu(x, approximate=True)


ACT2FN: Dict[str, ActivationFn] = {
    "gelu": _gelu_exact,  # Use exact GELU to match PyTorch
    "gelu_new": _gelu_approximate,  # Approximate GELU (legacy name)
    "gelu_approximate": _gelu_approximate,
    "relu": jax.nn.relu,
    "swish": jax.nn.swish,
    "silu": jax.nn.silu,
    "tanh": jnp.tanh,
