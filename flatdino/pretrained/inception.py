# SOURCE: https://github.com/willisma/diffuse_nnx/blob/main/eval/inception.py
...
URL = "https://github.com/willisma/diffuse_nnx/raw/refs/heads/main/eval/inception_v3_weights_fid.pickle"
...
MODEL_FILENAME = Path(_url_path).name or "inception_v3_weights_fid.pickle"
...
def load_inception_weights(force_download: bool = False, root: Path | None = None) -> Path:
    """Ensure the Inception V3 weights file is cached locally and return its path."""
...
class InceptionV3(nn.Module):
    ...
    def setup(self):
        if self.pretrained:
            weights_path = load_inception_weights()
            with open(weights_path, "rb") as f:
                self.params_dict = pickle.load(f)
            self.num_classes_ = 1000
            logging.info("InceptionV3: loaded pretrained weights")
        else:
    ...
    def __call__(self, x, train=True, rng=None):
        ...
        x = BasicConv2d(
            ...
            kernel_size=(3, 3),
            strides=(2, 2),
            params_dict=utils_get(self.params_dict, "Conv2d_1a_3x3"),
            dtype=self.dtype,
        )(x, train)
        x = BasicConv2d(
            out_channels=32,
            kernel_size=(3, 3),
            params_dict=utils_get(self.params_dict, "Conv2d_2a_3x3"),
            dtype=self.dtype,
        )(x, train)
        x = BasicConv2d(
            out_channels=64,
            kernel_size=(3, 3),
            padding=((1, 1), (1, 1)),
            params_dict=utils_get(self.params_dict, "Conv2d_2b_3x3"),
            dtype=self.dtype,
        )(x, train)
        ...
        x = BasicConv2d(
            out_channels=80,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "Conv2d_3b_1x1"),
            dtype=self.dtype,
        )(x, train)
        x = BasicConv2d(
            out_channels=192,
            kernel_size=(3, 3),
            params_dict=utils_get(self.params_dict, "Conv2d_4a_3x3"),
            dtype=self.dtype,
        )(x, train)
        x = nn.max_pool(x, window_shape=(3, 3), strides=(2, 2))
        x = InceptionA(
            pool_features=32, params_dict=utils_get(self.params_dict, "Mixed_5b"), dtype=self.dtype
        )(x, train)
        x = InceptionA(
            pool_features=64, params_dict=utils_get(self.params_dict, "Mixed_5c"), dtype=self.dtype
        )(x, train)
        x = InceptionA(
            pool_features=64, params_dict=utils_get(self.params_dict, "Mixed_5d"), dtype=self.dtype
        )(x, train)
        x = InceptionB(params_dict=utils_get(self.params_dict, "Mixed_6a"), dtype=self.dtype)(
            x, train
        )
        x = InceptionC(
            channels_7x7=128, params_dict=utils_get(self.params_dict, "Mixed_6b"), dtype=self.dtype
        )(x, train)
        x = InceptionC(
            channels_7x7=160, params_dict=utils_get(self.params_dict, "Mixed_6c"), dtype=self.dtype
        )(x, train)
        x = InceptionC(
            channels_7x7=160, params_dict=utils_get(self.params_dict, "Mixed_6d"), dtype=self.dtype
        )(x, train)
        x = InceptionC(
            channels_7x7=192, params_dict=utils_get(self.params_dict, "Mixed_6e"), dtype=self.dtype
        )(x, train)
        aux = None
        if self.aux_logits and train:
            aux = InceptionAux(
                num_classes=self.num_classes_,
                params_dict=utils_get(self.params_dict, "AuxLogits"),
                dtype=self.dtype,
            )(x, train)
        x = InceptionD(params_dict=utils_get(self.params_dict, "Mixed_7a"), dtype=self.dtype)(
            x, train
        )
        x = InceptionE(
            avg_pool, params_dict=utils_get(self.params_dict, "Mixed_7b"), dtype=self.dtype
        )(x, train)
        ...
        x = InceptionE(
            nn.max_pool, params_dict=utils_get(self.params_dict, "Mixed_7c"), dtype=self.dtype
        )(x, train)
        ...
        x = Dense(
            ...
            params_dict=utils_get(self.params_dict, "fc"),
            dtype=self.dtype,
        )(x)
        if self.aux_logits:
            return x, aux
...
class BasicConv2d(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        if self.params_dict is None:
            x = BatchNorm(
                epsilon=0.001, momentum=0.1, use_running_average=not train, dtype=self.dtype
            )(x)
        else:
            x = BatchNorm(
                epsilon=0.001,
...
class InceptionA(nn.Module):
    ...
    def __call__(self, x, train=True):
        branch1x1 = BasicConv2d(
            out_channels=64,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch1x1"),
            dtype=self.dtype,
        )(x, train)
        branch5x5 = BasicConv2d(
            out_channels=48,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch5x5_1"),
            dtype=self.dtype,
        )(x, train)
        branch5x5 = BasicConv2d(
            out_channels=64,
            kernel_size=(5, 5),
            padding=((2, 2), (2, 2)),
            params_dict=utils_get(self.params_dict, "branch5x5_2"),
            dtype=self.dtype,
        )(branch5x5, train)
        ...
        branch3x3dbl = BasicConv2d(
            out_channels=64,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch3x3dbl_1"),
            dtype=self.dtype,
        )(x, train)
        branch3x3dbl = BasicConv2d(
            out_channels=96,
            kernel_size=(3, 3),
            padding=((1, 1), (1, 1)),
            params_dict=utils_get(self.params_dict, "branch3x3dbl_2"),
            dtype=self.dtype,
        )(branch3x3dbl, train)
        branch3x3dbl = BasicConv2d(
            out_channels=96,
            kernel_size=(3, 3),
            padding=((1, 1), (1, 1)),
            params_dict=utils_get(self.params_dict, "branch3x3dbl_3"),
            dtype=self.dtype,
        )(branch3x3dbl, train)
        ...
        branch_pool = BasicConv2d(
            ...
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch_pool"),
            dtype=self.dtype,
        )(branch_pool, train)
...
class InceptionB(nn.Module):
    ...
    def __call__(self, x, train=True):
        branch3x3 = BasicConv2d(
            ...
            kernel_size=(3, 3),
            strides=(2, 2),
            params_dict=utils_get(self.params_dict, "branch3x3"),
            dtype=self.dtype,
        )(x, train)
...
class InceptionC(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch7x7 = BasicConv2d(
            ...
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch7x7_1"),
            dtype=self.dtype,
        )(x, train)
        branch7x7 = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(1, 7),
            padding=((0, 0), (3, 3)),
            params_dict=utils_get(self.params_dict, "branch7x7_2"),
            dtype=self.dtype,
        )(branch7x7, train)
        branch7x7 = BasicConv2d(
            out_channels=192,
            kernel_size=(7, 1),
            padding=((3, 3), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch7x7_3"),
            dtype=self.dtype,
        )(branch7x7, train)
        ...
        branch7x7dbl = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch7x7dbl_1"),
            dtype=self.dtype,
        )(x, train)
        branch7x7dbl = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(7, 1),
            padding=((3, 3), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch7x7dbl_2"),
            dtype=self.dtype,
        )(branch7x7dbl, train)
        branch7x7dbl = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(1, 7),
            padding=((0, 0), (3, 3)),
            params_dict=utils_get(self.params_dict, "branch7x7dbl_3"),
            dtype=self.dtype,
        )(branch7x7dbl, train)
        branch7x7dbl = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(7, 1),
            padding=((3, 3), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch7x7dbl_4"),
            dtype=self.dtype,
        )(branch7x7dbl, train)
        branch7x7dbl = BasicConv2d(
            out_channels=self.channels_7x7,
            kernel_size=(1, 7),
            padding=((0, 0), (3, 3)),
            params_dict=utils_get(self.params_dict, "branch7x7dbl_5"),
            dtype=self.dtype,
        )(branch7x7dbl, train)
...
class InceptionD(nn.Module):
    ...
    def __call__(self, x, train=True):
        branch3x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch3x3_1"),
            dtype=self.dtype,
        )(x, train)
        branch3x3 = BasicConv2d(
            out_channels=320,
            kernel_size=(3, 3),
            strides=(2, 2),
            params_dict=utils_get(self.params_dict, "branch3x3_2"),
            dtype=self.dtype,
        )(branch3x3, train)
        ...
        branch7x7x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "branch7x7x3_1"),
            dtype=self.dtype,
        )(x, train)
        branch7x7x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(1, 7),
            padding=((0, 0), (3, 3)),
            params_dict=utils_get(self.params_dict, "branch7x7x3_2"),
            dtype=self.dtype,
        )(branch7x7x3, train)
        branch7x7x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(7, 1),
            padding=((3, 3), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch7x7x3_3"),
            dtype=self.dtype,
        )(branch7x7x3, train)
        branch7x7x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(3, 3),
            strides=(2, 2),
            params_dict=utils_get(self.params_dict, "branch7x7x3_4"),
            dtype=self.dtype,
        )(branch7x7x3, train)
...
class InceptionE(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch3x3_a = BasicConv2d(
            ...
            kernel_size=(1, 3),
            padding=((0, 0), (1, 1)),
            params_dict=utils_get(self.params_dict, "branch3x3_2a"),
            dtype=self.dtype,
        )(branch3x3, train)
        branch3x3_b = BasicConv2d(
            ...
            kernel_size=(3, 1),
            padding=((1, 1), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch3x3_2b"),
            dtype=self.dtype,
        )(branch3x3, train)
        ...
        branch3x3dbl_a = BasicConv2d(
            ...
            kernel_size=(1, 3),
            padding=((0, 0), (1, 1)),
            params_dict=utils_get(self.params_dict, "branch3x3dbl_3a"),
            dtype=self.dtype,
        )(branch3x3dbl, train)
        branch3x3dbl_b = BasicConv2d(
            ...
            kernel_size=(3, 1),
            padding=((1, 1), (0, 0)),
            params_dict=utils_get(self.params_dict, "branch3x3dbl_3b"),
            dtype=self.dtype,
        )(branch3x3dbl, train)
...
class InceptionAux(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = BasicConv2d(
            out_channels=128,
            kernel_size=(1, 1),
            params_dict=utils_get(self.params_dict, "conv0"),
            dtype=self.dtype,
        )(x, train)
        x = BasicConv2d(
            out_channels=768,
            kernel_size=(5, 5),
            params_dict=utils_get(self.params_dict, "conv1"),
            dtype=self.dtype,
        )(x, train)
